...
#define EHCI_POWER_PATH "/sys/devices/platform/s5p-ehci/ehci_power"
#define HUB0_POWER_PATH "/sys/class/gpio/gpio195/value"
#define HUB1_POWER_PATH "/sys/class/gpio/gpio203/value"
#define HUB0_RESET_PATH "/sys/class/gpio/gpio131/value"
#define HUB1_RESET_PATH "/sys/class/gpio/gpio132/value"
...
static int ehci_power_on_off(int value)
{

	struct file * ehci_power_fd = NULL;
	int amt;

	ehci_power_fd = file_open(EHCI_POWER_PATH,  O_WRONLY, 0);	

	if(ehci_power_fd == NULL) 
	{
		printk(KERN_ERR " fail EHCI_POWER_PATH_fd\n");
		goto error_fd_open;
	}

		if(value == 0)
		{
          file_write(ehci_power_fd, 0, "0", 1); 
		}
		else
		{
          file_write(ehci_power_fd, 0, "1", 1);
		}
		  file_close(ehci_power_fd);
		  return 1;		  		

error_fd_open:
	if(ehci_power_fd != NULL) file_close(ehci_power_fd);		
	return 0;
}
static void all_usb_hub_reset_before(int value)
{

	struct file * usb_reset_fd = NULL;
	struct file * usb_power_fd = NULL;
	int amt1, amt2;

	if(value == 0)
	{
            gpio_set_value(GPIO_HUB1_3V3_EN_H,0);
	    gpio_set_value(GPIO_USB_HUB_1_nRST_H,1);
//	usb_reset_fd = file_open(HUB0_RESET_PATH,  O_WRONLY, 0);
//	usb_power_fd = file_open(HUB0_POWER_PATH,  O_WRONLY, 0);
	}
	else
	{
            gpio_set_value(GPIO_HUB2_3V3_EN_H,0);
            gpio_set_value(GPIO_USB_HUB_2_nRST_H,1);		
//	usb_reset_fd = file_open(HUB1_RESET_PATH,  O_WRONLY, 0);
//	usb_power_fd = file_open(HUB1_POWER_PATH,  O_WRONLY, 0);	
	}
/*	if(usb_reset_fd == NULL) 
	{
		printk(KERN_ERR "(before) fail HUB%d_RESET_fd\n",value);
		goto error_fd_open;
	}
	if(usb_power_fd == NULL)
	{
        printk(KERN_ERR "(before) fail HUB%d_POWER_fd\n",value);
		goto error_fd_open;
	}
gpio_set_value(GPIO_USB_HUB_2_nRST_H, 1);
    amt1 = file_write(usb_power_fd, 0, "0", 1); 
    amt2 = file_write(usb_reset_fd, 0, "1", 1);

error_fd_open:
	if(usb_reset_fd != NULL) file_close(usb_reset_fd);		
	if(usb_power_fd != NULL) file_close(usb_power_fd);
*/
	return;
}

static void all_usb_hub_reset_after(int value)
{	
	struct file * usb_reset_fd = NULL;
	struct file * usb_power_fd = NULL;
	int amt1, amt2;
...
if(value == 0)
{
            gpio_set_value(GPIO_HUB1_3V3_EN_H,1);
			mdelay(120);
            gpio_set_value(GPIO_USB_HUB_1_nRST_H,0);
//	usb_reset_fd = file_open(HUB0_RESET_PATH,  O_WRONLY, 0);
//	usb_power_fd = file_open(HUB0_POWER_PATH,  O_WRONLY, 0);
}
else
{
            gpio_set_value(GPIO_HUB2_3V3_EN_H,1);
			mdelay(120);
            gpio_set_value(GPIO_USB_HUB_2_nRST_H,0);
//	usb_reset_fd = file_open(HUB1_RESET_PATH,  O_WRONLY, 0);
//	usb_power_fd = file_open(HUB1_POWER_PATH,  O_WRONLY, 0);	
}
/*	if(usb_reset_fd == NULL) 
	{
		printk(KERN_ERR "(after) fail HUB%d_RESET_fd\n",value);
		goto error_fd_open;
	}
	if(usb_power_fd == NULL)
	{
        printk(KERN_ERR "(after) fail HUB%d_POWER_fd\n",value);
		goto error_fd_open;
	}

    amt1 = file_write(usb_power_fd, 0, "1", 1); 
    amt2 = file_write(usb_reset_fd, 0, "0", 1);
...
#define INFORMTOANDROID "/sys/class/android_usb/android0/informtoandroid"
#define INFORMTOANDROID_ENABLE "/sys/class/android_usb/android0/enable"

#define INFORM_ENABLE "/proc/driver/kodari_hub/otg"

int repeate_count = 0;
int check_event_occurs = 0;
int spin_nskang;
extern struct kobject *mykobjstd;

int exynos_inform_to_android_hsic_reset(int value)
{
	unsigned long flags;

	printk(KERN_ERR "=====  %s : %d =====<1>\n", __func__,repeate_count);	

    spin_lock(&spin_nskang);
	repeate_count++;
	if(repeate_count != value ) {
	    spin_unlock(&spin_nskang);		
		return;
	}
	if(check_event_occurs == 1) {
		spin_unlock(&spin_nskang);			
		return;
	}
    check_event_occurs = 1;
//	printk(KERN_ERR "=====  %s : %d =====<2>\n", __func__,repeate_count);	
	spin_unlock(&spin_nskang);	

//	struct file * informtoandroid_fd = NULL;
//	struct file * android_enable_fd = NULL;
//	int amt1, amt2;
//	char *imsi;
    sysfs_notify(mykobjstd, NULL, "notify");
//	informtoandroid_fd = file_open(INFORMTOANDROID,  O_WRONLY, 0);	
//	android_enable_fd = file_open(INFORMTOANDROID_ENABLE,  O_WRONLY, 0);

//	if(informtoandroid_fd == NULL) 
//	{
//		printk(KERN_ERR "INFORMTOANDROID open fail !!\n");
//		goto error_fd_open;
//	}
//	if(android_enable_fd == NULL)
//	{
//      printk(KERN_ERR "INFORM_ENABLE open fail !!\n");
	//	goto error_fd_open;
//	}
//	printk(KERN_ERR "=====  %s : %d =====<3>\n", __func__,repeate_count);	
    //printk(KERN_ERR "INFORMTOANDROID  1 !!\n");
//    amt2 = file_write(android_enable_fd, 0, "0", 1);
//    mdelay(100);
	//printk(KERN_ERR "INFORMTOANDROID  2!!\n");
//    amt1 = file_write(informtoandroid_fd, 0, "1", 1); 
    //mdelay(100);	
	//printk(KERN_ERR "INFORMTOANDROID  3!!\n");
//    amt2 = file_write(android_enable_fd, 0, "1", 1);
//    mdelay(100);
//    amt2 = file_write(android_enable_fd, 0, "0", 1);
//	printk(KERN_ERR "=====  %s : %d =====<5>\n", __func__,repeate_count);	
//error_fd_open:
//printk(KERN_ERR "INFORMTOANDROID  4!!\n");
//	if(informtoandroid_fd != NULL) file_close(informtoandroid_fd);
//printk(KERN_ERR "INFORMTOANDROID  5!!\n");
//	if(android_enable_fd != NULL) file_close(android_enable_fd);
    repeate_count = 0;
	return repeate_count;

}

void exynos_hsic_reset_control(int value)
{
	if(check_event_occurs != 1) return;
   printk(KERN_ERR "##000000 %s 000000##\n", __func__);			
	struct platform_device *pdev;

    all_usb_hub_reset_before(0);
    all_usb_hub_reset_before(1);
    udelay(10);	
	printk(KERN_ERR " ehci_power_on_off return: %d \n", ehci_power_on_off(0));	
    my_delay(500);		
//    my_delay(500);	
//    my_delay(500);		
//    my_delay(500);		
	u32 phypwr,rstcon, phyclk;	
	...
	phypwr = readl(EXYNOS4_PHYPWR);
			...
			rstcon = readl(EXYNOS4_RSTCON) | 0x03;				
...
//    my_delay(500);		
    my_delay(500);		
		all_usb_hub_reset_after(0);
		all_usb_hub_reset_after(1);
		udelay(10);	
		ehci_power_on_off(1);
		check_times = 0;
		check_event_occurs = 0;
		printk(KERN_ERR "%s check_event_occurs=0 \n",__func__); 
...
static int exynos_usb_phy_clock_disable(struct platform_device *pdev)
{
//nskang to fix debug port character broken issue.
