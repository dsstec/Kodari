...
#elif defined(CONFIG_FB_S5P_LMS480JC01) || defined(CONFIG_FB_S5P_OD102NA0D)
void s3cfb_cfg_gpio(struct platform_device *pdev)
{
	s3cfb_gpio_setup_24bpp(EXYNOS4_GPF0(0), 8, S3C_GPIO_SFN(2), S5P_GPIO_DRVSTR_LV4);
	s3cfb_gpio_setup_24bpp(EXYNOS4_GPF1(0), 8, S3C_GPIO_SFN(2), S5P_GPIO_DRVSTR_LV4);
...
int s3cfb_lcd_on(struct platform_device *pdev)
{
	...
	err = gpio_request_one(EXYNOS4_GPC0(1), GPIOF_OUT_INIT_HIGH, "GPC0");
	if (err) {
...
int s3cfb_lcd_on(struct platform_device *pdev)
{
	...
	err = gpio_request_one(EXYNOS4_GPX0(1), GPIOF_OUT_INIT_HIGH, "GPX0");
	if (err) {
	...
	gpio_set_value(EXYNOS4_GPX0(1), 1);
	msleep(10);
...
int s3cfb_lcd_off(struct platform_device *pdev)
{
	gpio_request_one(EXYNOS4_GPC0(1), GPIOF_OUT_INIT_LOW, "GPC0");	//lcd reset
	gpio_free(EXYNOS4_GPC0(1));
