	...
	struct snd_pcm_hw_params *params)
{
	...
	//ret = snd_soc_dai_set_sysclk(codec_dai, 0, 24576000, SND_SOC_CLOCK_OUT);
...
static int origen_ak4678_init_paiftx(struct snd_soc_pcm_runtime *rtd)
{
...
static struct snd_soc_dai_link origen_dai[] = {
	{
		...
		.init = origen_ak4678_init_paiftx,
		.ops = &origen_ops,
	},
};
...
static int __init origen_audio_init(void)
{
	...
	reg = __raw_readl(ETC6PUD);
	reg &= ~(3 << 2);
	reg |= (3 << 2);
	__raw_writel(reg, ETC6PUD);
...
MODULE_DESCRIPTION("ALSA SoC Driver for Origen Board");
MODULE_LICENSE("GPL");
