...
#ifdef CONFIG_KODARI_CAR_24BIT_LCD
int s3cfb_colorgain(struct s3cfb_global *ctrl)
{
	u32 cfg = 0;
	cfg = readl(ctrl->regs + 0x01C0); // colorgaincontrol
	cfg = 0x0A0280A0;  // initial value = 0x10040100
	writel(cfg, ctrl->regs + 0x01C0);

	cfg = readl(ctrl->regs + 0x000C); //VIDCON3 enable RGB-gain
	cfg = cfg & ~(0x04<<16);
	cfg = cfg | (0x04<<16);
	writel(cfg, ctrl->regs + 0x000C);
