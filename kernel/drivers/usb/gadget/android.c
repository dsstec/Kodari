...
//nskang
	int informtoandroid = 0;
#if 1 // SET_ADB_SERIAL_NUMBER
extern char eth_mac_string[16];
#endif
...
static ssize_t informtoandroid_store(struct device *dev, struct device_attribute *attr, char *buf, size_t size) {

	sscanf(buf, "%d", &informtoandroid);

	
	//printk("[%s] >>>>>>>>>>>>>>>>>>> STORE[%d]",__func__,informtoandroid);
	return size;
}

static ssize_t informtoandroid_show(struct device *pdev, struct device_attribute *attr,
			   char *buf)
{
    //informtoandroid = 1;
	if(check_event_occurs == 1)
		informtoandroid = 1;
	else informtoandroid = 0;
	//printk("[%s] >>>>>>>>>>>>>>>>>>> SHOW[%d]",__func__,informtoandroid);
	return sprintf(buf, "%d", informtoandroid);
}
...
static DEVICE_ATTR(informtoandroid, /*S_IRUGO | S_IWUSR*/ 0777, informtoandroid_show, informtoandroid_store);
...
static struct device_attribute *android_usb_attributes[] = {
	...
	&dev_attr_enable,
	&dev_attr_state,
	&dev_attr_informtoandroid,	//nskang
	NULL
};
...
#if 1 // SET_ADB_SERIAL_NUMBER
	// esshin set different ADB serial number
	sprintf(serial_string, "KODARI%s", eth_mac_string);
	printk("serial_string: %s\n", serial_string);
#else
	//strncpy(serial_string, "0123456789ABCDEF", sizeof(serial_string) - 1);
	//leehc modify
       strncpy(serial_string, "KODARI_01093745520", sizeof(serial_string) - 1);
#endif
...
static int __init init(void)
{
    ...
    mykobjstd = kzalloc(sizeof(*mykobjstd), GFP_KERNEL);
    if (mykobjstd) {
        kobject_init(mykobjstd, &mytype);
        if (kobject_add(mykobjstd, NULL, "%s", "sstd_hsic_status")) {
             err = -1;
             printk("kobject_add() failed\n");
             kobject_put(mykobjstd);
             mykobjstd = NULL;
        }
        err = 0;
    }
