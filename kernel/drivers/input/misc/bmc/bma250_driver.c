		...
		struct bma250acc *acc)
{
	...
	if (client == NULL) {
		...
		x = BMA250_GET_BITSLICE(data[0], BMA250_ACC_X_LSB)
			|(BMA250_GET_BITSLICE(data[1], BMA250_ACC_X_MSB)
					<<BMA250_ACC_X_LSB__LEN);
		x = x << (sizeof(short)*8-(BMA250_ACC_X_LSB__LEN
					+ BMA250_ACC_X_MSB__LEN));
		x = x >> (sizeof(short)*8-(BMA250_ACC_X_LSB__LEN
					+ BMA250_ACC_X_MSB__LEN));
		y = BMA250_GET_BITSLICE(data[2], BMA250_ACC_Y_LSB)
			| (BMA250_GET_BITSLICE(data[3], BMA250_ACC_Y_MSB)
					<<BMA250_ACC_Y_LSB__LEN);
		y = y << (sizeof(short)*8-(BMA250_ACC_Y_LSB__LEN
					+ BMA250_ACC_Y_MSB__LEN));
		y = y >> (sizeof(short)*8-(BMA250_ACC_Y_LSB__LEN
					+ BMA250_ACC_Y_MSB__LEN));

		z = BMA250_GET_BITSLICE(data[4], BMA250_ACC_Z_LSB)
			| (BMA250_GET_BITSLICE(data[5], BMA250_ACC_Z_MSB)
					<<BMA250_ACC_Z_LSB__LEN);
		z = z << (sizeof(short)*8-(BMA250_ACC_Z_LSB__LEN
					+ BMA250_ACC_Z_MSB__LEN));
		z = z >> (sizeof(short)*8-(BMA250_ACC_Z_LSB__LEN
					+ BMA250_ACC_Z_MSB__LEN));
	}
	/* SDKIM Swap and flip for mv4412-TAB */
#if defined(CONFIG_BMA250_KODARI_HANDSET)
	acc->x =  x;
	acc->y =  y;
	acc->z =  z;
#elif defined(CONFIG_BMA250_KODARI_CARSET)
	acc->x =  x;
	acc->y =  y;
	acc->z =  z;
#else
#error you must selet one type for bma250 car or hand
#endif
		...
		const struct i2c_device_id *id)
{
	...
	regulator = regulator_get(NULL, "MAC_2V0");
	if (IS_ERR(regulator)) {
	    printk("[%s] regulator_get fail : MAC_2V0\n", __func__);
	    return -ENODEV;
	}

	ret = regulator_enable(regulator);
	if(ret < 0)
	    printk("[%s] regulator_enable fail : MAC_2V0\n", __func__);
...
static int bma250_driver_suspend(struct i2c_client *client, pm_message_t mesg)
{
	struct regulator *regulator;
	int ret = 0;
	...
	if (regulator_is_enabled(regulator)) {
		...
		if(ret < 0)
			printk("[%s] regulator_force_disable fail : MAC_2V0\n", __func__);
	}
...
static int bma250_driver_resume(struct i2c_client *client)
{
	struct regulator *regulator;
	int ret = 0;
...
static void bma250_early_suspend(struct early_suspend *h)
{
	...
	printk(KERN_INFO "bma250_early_suspend \n");
...
static void bma250_late_resume(struct early_suspend *h)
{
	...
	printk(KERN_INFO "bma250_late_resume \n");
...
static struct i2c_driver bma250_driver = {
	...
	.suspend = bma250_driver_suspend,
	.resume = bma250_driver_resume,
};
