...
/* generic */
#define BMM_MAX_RETRY_I2C_XFER (2) // (100)
#define BMM_MAX_RETRY_WAKEUP (2) // (50)
...
static int bmm_wakeup(struct i2c_client *client)
{
	...
	while (try_times) {
		...
		printk("\tREG:%x value:%x dummy:%x\n", BMM_REG_NAME(POWER_CNTL), value, dummy);
		mdelay(500);
		try_times--;
	}
...
static void bmm_work_func(struct work_struct *work)
{
	...
	x =client_data->value.datax;
	y = client_data->value.datay;
	z = client_data->value.dataz;


//	PDEBUG("input_report_abs x=%i,y=%i,z=%i", x,y,z);
	
	input_report_abs(client_data->input, ABS_X, x);
	input_report_abs(client_data->input, ABS_Y, y);
	input_report_abs(client_data->input, ABS_Z, z);
