...
#ifdef CONFIG_HAS_EARLYSUSPEND
static void tsc2004_early_suspend(struct early_suspend *h);
static void tsc2004_late_resume(struct early_suspend *h);
#endif
...
//dev_err(&tsc->client->dev, "soo tsc2004 write word data!\n");
...
//dev_err(&tsc->client->dev, "soo tsc2004 write cmd!\n");
...
//dev_err(&ts->client->dev, "soo tsc2004 prepare for reading! 1 \n");
...
//dev_err(&ts->client->dev, "soo tsc2004 prepare for reading! 2 \n");
...
//dev_err(&ts->client->dev, "soo tsc2004 prepare for reading! 3 \n");
...
//dev_err(&ts->client->dev, "soo tsc2004 prepare for reading! 4 \n");
...
static u32 tsc2004_calculate_pressure(struct tsc2004 *tsc, struct ts_event *tc)
{
	...
	if (likely(tc->x && tc->z1)) {
		...
		rt /= tc->z1;
		rt = (rt + 2047) >> 12;
//		dev_err(&tsc->client->dev, "soo tsc2004 calculate prepare pressure!-- in");		
	}

//	dev_err(&tsc->client->dev, "soo tsc2004 calculate prepare pressure! %i\n", rt);
...
//dev_err(&tsc->client->dev, "soo tsc2004 send up event! \n");
...
//dev_err(&ts->client->dev, "soo tsc2004 work get_pendown_state! %d \n", ts->get_pendown_state);
			ts->pendown = false;
			goto out;
		}

		dev_dbg(&ts->client->dev, "pen is still down\n");
//		dev_err(&ts->client->dev, "pen is still down\n");
	}

	tsc2004_read_values(ts, &tc);

//dev_err(&ts->client->dev, "soo tsc2004 work! \n");
...
//dev_err(&ts->client->dev, "soo tsc2004 work MAX_12BIT! %d \n", rt);
		goto out;
...
//leehc
#if defined(CONFIG_TOUCHSCREEN_KODARI_HANDSET)
		input_report_abs(input, ABS_X, 0xFF - tc.x);
		input_report_abs(input, ABS_Y, 0xFF - tc.y);
#elif defined(CONFIG_TOUCHSCREEN_KODARI_CARSET)
		input_report_abs(input, ABS_X, tc.x);
		input_report_abs(input, ABS_Y, tc.y);
#else
#error you must selet one type for touchscreen car or hand
		...
		dev_dbg(&ts->client->dev, "point(%4d,%4d), pressure (%4u)\n",
			tc.x, tc.y, rt);
//		dev_err(&ts->client->dev, "point(%4d,%4d), pressure (%4u)\n",
//			tc.x, tc.y, rt);
 ...
 out:
//dev_err(&ts->client->dev, "soo tsc2004 work - out! \n");
...
//dev_err(&ts->client->dev, "soo tsc2004 irq! \n");

	if (!ts->get_pendown_state || likely(ts->get_pendown_state())) {
		disable_irq_nosync(ts->irq);
//dev_err(&ts->client->dev, "soo tsc2004 irq get_pendown_state! \n");
		schedule_delayed_work(&ts->work,
...
//dev_err(&ts->client->dev, "soo tsc2004 free irq! \n");
...
//dev_err(&client->dev, "soo tsc2004 probe! \n");
	...
	dev_err(&client->dev, "register irq %d \n", ts->irq);
	...
	ts->early_suspend.suspend = tsc2004_early_suspend;
	ts->early_suspend.resume = tsc2004_late_resume;
	register_early_suspend(&ts->early_suspend);
#endif

	i2c_set_clientdata(client, ts);
	dev_err(&client->dev, "device add %s \n", ts->phys);
...
//dev_err(&client->dev, "soo tsc2004 err_free_irq! \n");
	tsc2004_free_irq(ts);
	if (pdata->exit_platform_hw)
		pdata->exit_platform_hw();
 err_free_mem:
//dev_err(&client->dev, "soo tsc2004 err_free_mem! \n");
...
//dev_err(&client->dev, "soo tsc2004 remove! \n");
...
static void __tsc2004_disable(struct tsc2004 *ts)
{
	/*tsc2004_stop_scan( ts);
	disable_irq(ts->irq);
...
/* must be called with ts->mutex held */
static void __tsc2004_enable(struct tsc2004 *ts)
{
...
static void tsc2004_early_suspend(struct early_suspend *h)
{
	struct tsc2004 *ts = container_of(h, struct tsc2004, early_suspend);
